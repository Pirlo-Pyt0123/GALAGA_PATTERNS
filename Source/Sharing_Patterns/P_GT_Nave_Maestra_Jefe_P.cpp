...
AP_GT_Nave_Maestra_Jefe_P::AP_GT_Nave_Maestra_Jefe_P()
{
    ...
    static ConstructorHelpers::FObjectFinder<UStaticMesh> MeshAsset(TEXT("StaticMesh'/Game/Mallas_Nave_Enemiga/VARIOS_A/Nodriza1.Nodriza1'"));
...
void AP_GT_Nave_Maestra_Jefe_P::BeginPlay()
{
    Super::BeginPlay();
...
void AP_GT_Nave_Maestra_Jefe_P::Tick(float DeltaTime)
{
...
void AP_GT_Nave_Maestra_Jefe_P::ActualizarEstadoDeLuzInfrarroja()
{
    if (LuzInfraroja)
    {
...
void AP_GT_Nave_Maestra_Jefe_P::ComenzarGiro()
{
    bGirando = true;
}

void AP_GT_Nave_Maestra_Jefe_P::GirarNave(float DeltaTime)
{
...
void AP_GT_Nave_Maestra_Jefe_P::CambiarEstado(IP_ES_Int_Estado_P* NuevoEstado)
{
    if (EstadoActual)
    {
...
void AP_GT_Nave_Maestra_Jefe_P::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
    Super::SetupPlayerInputComponent(PlayerInputComponent);
}

void AP_GT_Nave_Maestra_Jefe_P::NotifyActoBeginOverlap(AActor* OtherActor)
{
...
void AP_GT_Nave_Maestra_Jefe_P::Disparar_Nave_Maestra(const FVector& PawnLocation)
{
...
void AP_GT_Nave_Maestra_Jefe_P::Actualizar_Suscripcion(APO_Luz_Infraroja* _Escaner, const FVector& PawnLocation)
{
    if (bEscaneando)
    {
        DesactivarEscaner();
        ...
        GetWorld()->GetTimerManager().SetTimer(TimerHandle_ReactivarEscaner, this, &AP_GT_Nave_Maestra_Jefe_P::ReactivarEscaner, 15.0f, false);
    }
}

void AP_GT_Nave_Maestra_Jefe_P::Establecer_Suscrip(APO_Luz_Infraroja* O_Observador)
{
    if (!O_Observador) {
...
void AP_GT_Nave_Maestra_Jefe_P::Destruir_Suscripcion()
{
    if (LuzInfraroja)
    {
...
void AP_GT_Nave_Maestra_Jefe_P::Cambiar_Estrategia()
{
...
void AP_GT_Nave_Maestra_Jefe_P::ActivarEscaner()
{
    if (!LuzInfraroja)
    {
...
void AP_GT_Nave_Maestra_Jefe_P::DesactivarEscaner()
{
    if (LuzInfraroja)
    {
...
void AP_GT_Nave_Maestra_Jefe_P::ReactivarEscaner()
{
...
void AP_GT_Nave_Maestra_Jefe_P::Componente_Destruccion()
{
    Destruir_Suscripcion();
    GetWorld()->GetTimerManager().ClearTimer(TimerHandle_Disparo);
...
void AP_GT_Nave_Maestra_Jefe_P::Recibir_Danio(float Danio)
{
    Life -= Danio;
}
